...
The version-5 JSON output has one complete raw result per requested topology and concurrency level, plus compact factual summaries. Every new document records the structured `effective_database_pool.max_connections_per_instance`, derived from the same `DB_MAX_CONNECTIONS` value supplied to the service instances; no service environment dump is stored. The legacy `database_pool_assumptions` field remains for v5 consumers, but is now derived from that structured value rather than configured independently. The final report reader also supports pre-existing v5 campaign artifacts through their numeric `campaign_pool` provenance. Every level records scenario/seed, operation counts, latency, throughput, classifications, per-instance request counts, metrics snapshots, explicit `metrics_collection_valid`, SQL verification, validity, environment and limitations. Account-pool levels additionally record pool and phase metadata. Version 5 deliberately supersedes version 4: use the paired v5 summarizer rather than passing v5 output to a schema-v4 consumer. Interpret `valid: true` as the workload and SQL checks passing in that environment; do not treat it as a capacity or production-performance guarantee. Raw results are ignored by default.
