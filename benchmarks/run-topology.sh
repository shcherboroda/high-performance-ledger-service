...
instances="${1:-2}"
case "$instances" in 1|2) ;; *) echo "usage: $0 [1|2]" >&2; exit 2;; esac
...
for url in "${urls[@]}"; do
  for attempt in $(seq 1 60); do curl -fsS "$url/ready" >/dev/null && break; sleep 1; done
  curl -fsS "$url/ready" >/dev/null || { echo "$url did not become ready" >&2; exit 1; }
done
SERVICE_URLS="$(IFS=,; echo "${urls[*]}")" BENCHMARK_ALLOW_DESTRUCTIVE=1 \
  cargo run -p ledger-benchmarks --release -- \
  --scenario "${BENCHMARK_SCENARIO:-independent}" --instance-levels "$instances" \
  --output "benchmark-results/topology-$instances.json" "${@:2}"
