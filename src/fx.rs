...
pub async fn select_exchange_rate(
    ...
    .fetch_all(pool)
    .await
    .map_err(|_| ConfigurationError::RateUnavailable)?;
    match rows.as_slice() {
...
pub async fn select_fee_rule(
    ...
    let pair = pair_fee_rows(pool, source_currency, destination_currency, operation_time).await?;
    match pair.as_slice() {
        [rule] => return Ok(rule.clone()),
        [_, ..] => return Err(ConfigurationError::FeeRuleAmbiguous),
        [] => {}
    }
    let defaults = default_fee_rows(pool, operation_time).await?;
...
async fn pair_fee_rows(
    ...
    .fetch_all(pool)
    .await
    .map_err(|_| ConfigurationError::FeeRuleUnavailable)
}
...
mod tests {
    ...
    #[test]
    fn bigint_intermediate_exceeds_i128() {
        let rate = ExactRate::parse("999999999999999999.999999999999").unwrap();
        assert!(calculate_destination(i64::MAX, 18, 18, &rate).is_err());
    }
}
